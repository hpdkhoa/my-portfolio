#!/usr/bin/env python3
"""
fill_portfolio.py — auto-fill the public portfolio from your PRIVATE repos + benchmark results.
...
def inject_measured(md_path, marker, blocks):
    ...
    if not content:
        return False
    payload = ("\n### Measured results\n\n"
               "*Auto-rendered from `benchmarks/results/measured.json` — every number below "
...
def main():
    ...
    if stale:
        ...
        for s_ in stale:
            print("   -", s_)
        print("GitHub renders the .md files directly — simplest fix is to delete the .html "
              "twins, or regenerate them with your original converter.")
